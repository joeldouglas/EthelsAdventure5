...
public class GachaBehaviour : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI prizeText;
    [SerializeField] private Image prizeImage;
    [SerializeField] private GameObject SlotMachineUI; // The Main Parent of the Gacha Visuals
    ...
    // Reference to the manager to tell it we are done
    private FightManager fightManager; 

    void Start()
    {
        // Automatically find the manager in the scene
        fightManager = Object.FindAnyObjectByType<FightManager>();
        
        // Ensure Gacha is hidden at start
        if(SlotMachineUI != null) SlotMachineUI.SetActive(false);
    }

    // Called by FightManager when player wins
    public void InitializeGacha()
    {
        ...
        // Note: The player still needs to press the 'Spin' button (handled by UI Button OnClick)
        // OR you can auto-spin here by calling SpinSlotMachine();
    }
    ...
    public void SpinSlotMachine()
    {
        if (isSpinning || !isEnabled || !canSpin)
        {
            Debug.Log("GACHA: Machine is busy or disabled.");
            return;
        }
    ...
    private IEnumerator WheelSpinEffect()
    {
        ...
        while (timeTracker < spinDuration)
        {
            ...
            if (progress > 0.8f) currentFlashDelay = Mathf.Lerp(0.05f, 0.6f, (progress - 0.8f) / 0.2f);
    ...
    private IEnumerator ShowPrizeSequence(int tierIndex)
    {
        ...
        if (maskLibrary.Count == 0)
        {
            Debug.LogError("GACHA: Mask Library is empty!");
            isSpinning = false;
            yield break;
        }
        ...
        // Show the Prize Panel
        if(prizePanel != null) prizePanel.SetActive(true);

        // Turn on the selection buttons in the Team Tray
        TeamManager.Instance.SetTrayVisibility(true);
        foreach(var slot in TeamManager.Instance.slotUIs)
        {
    ...
    public void TrashCurrentPrize()
    {
        Debug.Log("GACHA: Prize trashed.");
        CleanupGacha();

        // Notify FightManager that we are done
    ...
    {
        if(prizePanel != null) prizePanel.SetActive(false);
        if(prizeText != null) prizeText.text = "";
        ...
        // Hide the whole Slot Machine UI now that we are done
        if(SlotMachineUI != null) SlotMachineUI.SetActive(false);
    }
}